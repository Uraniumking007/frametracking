// Server-side WFCD items resolver using pre-generated dataset
// This will be updated once itemsMap.json is available
// For now, we'll create a placeholder that will work with the structure

// Placeholder - will be replaced when itemsMap.json is added
...
export async function resolveItemName(identifier: string): Promise<string> {
  ...
  // Fallback to cleaning the identifier
...
export async function resolveItem(identifier: string): Promise<{
  ...
  const name = ITEM_NAME_MAP[identifier]
  if (name) {
    return {
...
export async function resolveMultipleItems(
  ...
  for (const identifier of identifiers) {
    ...
    resolved[identifier] =
      ITEM_NAME_MAP[identifier] || cleanItemIdentifier(identifier)
  }
